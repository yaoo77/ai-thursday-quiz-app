...
- **全20問のクイズ**（4择10問 + 画像2择7問 + 文章2择3問）
- **チーム機能**（最大20チーム）
- **メンバー登録・ログイン機能**（1チーム5〜10人、最大200人）
...
マスター（司会者）が問題の進行を制御し、チームメンバーが同時に回答するシステムです。

### 主要機能

#### 1. マスター選定
- チームの最初のメンバーが自動的にマスターになる
- マスターには「マスター」バッジが表示される
- マスターのみがクイズを開始できる

#### 2. マスターコントロール
- **回答状況の表示**: 「X / Y人が回答済み」とリアルタイムで表示
- **次の問題へボタン**: 全員が回答済みの場合のみ有効
- **強制的に次へボタン**: いつでも押せる（確認ダイアログ付き）

#### 3. リアルタイム同期
- マスターがクイズを開始すると、全メンバーが自動的にクイズ画面に遷移
- マスターが問題を進めると、全メンバーの画面が自動更新
- メンバーが回答すると、マスターの画面に即座に反映

#### 4. マスター委譲
- マスター以外のメンバーが「マスター権限を引き継ぐ」ボタンを押せる
- 委譲時に確認ダイアログを表示
- 委譲後、すぐにマスターコントロールが使用可能

### メリット
- **不正行為の防止**: 全員が同じタイミングで問題を見るため、回答を見てから仲間に教えることができない
- **リアルタイム性**: マスターの操作が即座に全メンバーに反映される
- **柔軟性**: マスター委譲により、マスターが途中で抜けても継続可能
